...
app.post('/auth/register', async (req, res) => {
  ...
  try {
    ...
    const [rows] = await pool.query('SELECT * FROM F_USUARIO WHERE USUARIO_ID = ?', [result.insertId]);
    const usuario = rows[0];
    const token = jwt.sign({ id: usuario.USUARIO_ID, perfil: usuario.F_USUARIO_ID_USUARIO_PERFIL }, process.env.JWT_SECRET, { expiresIn: '1h' });
...
app.put('/auth/usuarios/:id', async (req, res) => {
  ...
  if (cpf) {
    ...
    const [cpfRows] = await pool.query('SELECT 1 FROM F_USUARIO WHERE USUARIO_CPF = ? AND USUARIO_ID != ?', [cpf, id]);
    if (cpfRows.length) return res.status(409).json({ error: 'CPF já cadastrado' });
  ...
  if (senha) {
    const hash = await bcrypt.hash(senha, 10);
    fields.push('USUARIO_SENHA = ?'); values.push(hash);
  }
  ...
  try {
    const [result] = await pool.query(`UPDATE F_USUARIO SET ${fields.join(', ')} WHERE USUARIO_ID = ?`, values);
    if (result.affectedRows === 0) return res.status(404).json({ error: 'Usuário não encontrado' });

    const [rows] = await pool.query('SELECT * FROM F_USUARIO WHERE USUARIO_ID = ?', [id]);
    const usuario = rows[0];
...
app.delete('/auth/usuarios/:id', async (req, res) => {
  ...
  try {
    const [result] = await pool.query('DELETE FROM F_USUARIO WHERE USUARIO_ID = ?', [id]);
